...
final class SessionStoreTests: XCTestCase {
    ...
    func testUserPromptSubmitClearsPreviousTurnToolEventsAndAssistantMessages() throws {
        let sessionId = "turn-reset-\(UUID().uuidString)"
        let store = SessionStore.shared

        let session = try XCTUnwrap(store.process(makeEvent(
            sessionId: sessionId,
    ...
    func testDisplaySessionNumbersRenumberAfterDismissal() throws {
        let store = SessionStore.shared
        let cwd = "/tmp/notchi"

        let first = try XCTUnwrap(store.process(makeEvent(
        ...
        let second = try XCTUnwrap(store.process(makeEvent(
        ...
        let third = try XCTUnwrap(store.process(makeEvent(
